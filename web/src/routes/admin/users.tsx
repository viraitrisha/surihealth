...
function AdminUsersPage() {
  ...
  const handleToggleBlock = async (
    ...
    try {
      ...
      toast({
        title: !currentlyBlocked ? 'Gebruiker geblokkeerd' : 'Gebruiker gedeblokkeerd',
        type: 'success',
      });
      router.invalidate();
    } catch (err: any) {
  ...
  const handleRoleToggle = async (
    ...
    try {
      ...
      toast({
        title: newRole === 'admin' ? 'Promoot tot Admin' : 'Gedegradeerd naar Gebruiker',
        type: 'success',
      });
      router.invalidate();
    } catch (err: any) {
  ...
  const handleDeleteUser = async (userId: string, email: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Alle gekoppelde data zijn succesvol geschoond.',
        type: 'success',
      });
      router.invalidate();
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex">
      ...
      <main className="flex-1 ml-72 p-10 pt-0 space-y-8 animate-in fade-in duration-150">
        <div className="h-6" />

        {/* Banner */}
        <section className="bg-gradient-to-r from-[#1A756A] to-[#13574e] rounded-3xl p-8 text-white shadow-lg flex flex-col md:flex-row justify-between gap-5">
          <div>
            <div className="flex items-center gap-3">
              <div className="bg-white/20 p-3 rounded-2xl">
                <Users className="w-8 h-8" />
              </div>
              <h1 className="text-3xl font-black">Gebruikersbeheer</h1>
            </div>
            <p className="mt-3 text-white/80 text-sm">
              Beheer accounts, blokkeer gebruikers of wijzig rollen binnen het SuriHealth platform.
            </p>
          </div>
          <div className="bg-white/10 rounded-2xl px-5 py-4 text-sm flex items-center gap-4 self-start md:self-center">
            <Users className="w-4 h-4 text-white/80" />
            <span className="font-bold">{totalUsers} Totaal</span>
          </div>
        </section>

        {/* Stat cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <StatCard icon={Users} color="teal" label="Gebruikers" value={totalUsers} />
          <StatCard icon={ShieldCheck} color="purple" label="Beheerders" value={adminCount} />
          <StatCard icon={ShieldAlert} color="red" label="Geblockt" value={blockedCount} />
        </div>

        {/* Search */}
        <div className="bg-white border rounded-3xl p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="relative w-full md:w-96">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#1A756A] outline-none bg-gray-50 text-gray-900 transition-all"
            />
          </div>
                  ...
                  displayedUsers.map((user: any) => (
                    <tr key={user.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-xs text-gray-500">
                        {new Date(user.createdAt).toLocaleDateString('nl-SR', {
                          day: '2-digit',
                          month: '2-digit',
                          year: 'numeric',
                        })}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            type="button"
                            disabled={
                              loadingBlockId === user.id ||
                              (isProtected(user.email) && !user.blocked)
                            }
                            onClick={() =>
                              handleToggleBlock(user.id, user.blocked, user.email)
                            }
                          ...
                          <button
                            ...
                            onClick={() =>
                              handleRoleToggle(user.id, user.role, user.email)
                            }
                            className={`inline-flex items-center justify-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer focus:outline-none ${
                              isProtected(user.email)
                                ? 'bg-gray-100 text-gray-400 cursor-not-allowed shadow-none'
          ...
          {totalPages > 1 && (
            <div className="border-t border-gray-100 px-6 py-3 flex items-center justify-between text-xs">
              <span className="text-gray-500 font-medium">
                Weergave {startIndex + 1}–{Math.min(endIndex, totalFiltered)} van{' '}
                {totalFiltered} gebruikers
              </span>
...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200 flex items-center gap-5">
